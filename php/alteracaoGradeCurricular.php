...
if ($status == 0) {
    ...
    // Id discilpina null para update de intervalo
    $id_disciplina_nulo = null;
    ...
    while ($array_aula_escola = $sql_select_padrao->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($id_disciplina == -1) {
            $sql_update_intervalo = $conn->prepare("UPDATE grade_curricular SET fk_id_disciplina_grade_curricular = :id_disciplina WHERE  fk_id_dia_semana_grade_curricular = :id_dia AND fk_id_aula_escola_grade_curricular = :id_aula AND fk_id_turma_grade_curricular = :id_turma");            
            $sql_update_intervalo->bindParam(':id_disciplina', $id_disciplina_nulo, PDO::PARAM_STR);
            $sql_update_intervalo->bindParam(':id_dia', $id_dia, PDO::PARAM_STR);
            $sql_update_intervalo->bindParam(':id_aula', $id_aula, PDO::PARAM_STR);
            $sql_update_intervalo->bindParam(':id_turma', $id_turma, PDO::PARAM_STR);
            $sql_update_intervalo->execute();

            if ($sql_update_intervalo->rowCount()) {
                $status += 1;
            }
        } else {
            $sql_update_grade = $conn->prepare("UPDATE grade_curricular SET fk_id_disciplina_grade_curricular = :id_disciplina WHERE  fk_id_dia_semana_grade_curricular = :id_dia AND fk_id_aula_escola_grade_curricular = :id_aula AND fk_id_turma_grade_curricular = :id_turma");
            $sql_update_grade->bindParam(':id_dia', $id_dia, PDO::PARAM_STR);
            $sql_update_grade->bindParam(':id_aula', $id_aula, PDO::PARAM_STR);
            $sql_update_grade->bindParam(':id_disciplina', $id_disciplina, PDO::PARAM_STR);
            $sql_update_grade->bindParam(':id_turma', $id_turma, PDO::PARAM_STR);
            $sql_update_grade->execute();

            if ($sql_update_grade->rowCount()) {
                $status += 1;
            }
        }
    }
    ...
    ?>
            <script>
                alert('Grade Alterada com sucesso !!');
        ...
        ?>
            <script>
                alert('Erro na Alteraçãp, nenhum dado alterado!!');
                window.location = '../homeDiretor.html.php';
            </script>
        <?php
        } else {
