  ...
  $image_file = $_FILES["txt_file"]["name"];
	...
	if ($image_file) {
		...
		if($size > $tamanho) {
			...
			$query = $conn->prepare("INSERT INTO professor (nome,foto, rg, cpf, cep, numero, complemento, email, senha, celular, telefone, fk_id_tipo_usuario_professor, fk_id_escola_professor)
            VALUES ('{$nome}', '{$nome_imagem}', '{$rg}', '{$cpf}', '{cep}', '{numero}', '{complemento}', '{$email}', '{$senha}', '{$celular}','{$telefone}','5' ,'{$id_escola}')");
